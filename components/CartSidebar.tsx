...
const CartSidebar: React.FC<CartSidebarProps> = ({ isOpen, onClose, cart, onUpdateQuantity, onRemoveItem }) => {
  ...
  const handleCheckout = () => {
    ...
    let message = `*Novo Pedido Favela +Q Chick*\n\n`;
    cart.forEach(item => {
      message += `- ${item.name} (R$ ${item.price.toFixed(2)}) x${item.quantity}\n`;
    });
    message += `\n*Total: R$ ${total.toFixed(2)}*`;

    const whatsappUrl = `https://api.whatsapp.com/send/?phone=5511977668767&text=${encodeURIComponent(message)}`;
    ...
    <AnimatePresence>
      {isOpen && (
        <>
          ...
          >
            {/* Header */}
            <div className="bg-[#E8FF00] text-black h-16 flex items-center justify-between px-6 shrink-0">
                ...
                cart.map(item => (
                  <div key={item.id} className="bg-[#111] border border-[#222] rounded-xl p-3 flex gap-4 relative group">
                    ...
                    <div className="flex-1 flex flex-col justify-center">
                      ...
                      <p className="text-[#E8FF00] font-bold text-sm mt-1">R$ {item.price.toFixed(2)}</p>
            ...
            {cart.length > 0 && (
              <div className="border-t border-[#222] p-6 bg-[#0a0a0a] shrink-0">
                <div className="flex justify-between items-center mb-6">
                  ...
                  <span className="text-[#E8FF00] text-2xl font-bold">R$ {total.toFixed(2)}</span>
                </div>
                
                <button 
                  onClick={handleCheckout}
                  className="w-full bg-[#E8FF00] hover:bg-white text-black py-4 rounded-xl font-bold uppercase tracking-widest transition-all flex items-center justify-center gap-3 relative group"
                >
                  ...
                  <div className="absolute right-4 w-8 h-8 bg-black rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                    <div className="w-2 h-2 bg-[#E8FF00] rounded-full" />
                  </div>
                </button>
              </div>
            )}
